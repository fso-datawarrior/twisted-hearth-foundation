...
do $$
begin
  if not exists (select 1 from pg_policies where schemaname='storage' and tablename='objects' and policyname='gallery_read_public') then
    create policy "gallery_read_public" on storage.objects for select using ( bucket_id = 'gallery' );
  end if;
  if not exists (select 1 from pg_policies where schemaname='storage' and tablename='objects' and policyname='gallery_insert_own') then
    create policy "gallery_insert_own" on storage.objects for insert with check (
      bucket_id = 'gallery' and auth.role() = 'authenticated'
      and split_part(name, '/', 1) = auth.uid()::text
    );
  end if;
  if not exists (select 1 from pg_policies where schemaname='storage' and tablename='objects' and policyname='gallery_delete_owner') then
    create policy "gallery_delete_owner" on storage.objects for delete using (
      bucket_id = 'gallery' and auth.uid()::text = split_part(name, '/', 1)
    );
  end if;
end$$;