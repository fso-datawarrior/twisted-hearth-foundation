...
DROP POLICY IF EXISTS "gb_delete_owner" ON public.guestbook;
DROP POLICY IF EXISTS "gb_insert_auth" ON public.guestbook;
DROP POLICY IF EXISTS "gb_select_all" ON public.guestbook;

-- Update the existing guestbook table structure
...
-- Create new RLS policies for guestbook table
...
DROP TRIGGER IF EXISTS update_guestbook_updated_at ON public.guestbook;
CREATE TRIGGER update_guestbook_updated_at
  BEFORE UPDATE ON public.guestbook
  FOR EACH ROW
  EXECUTE FUNCTION public.update_updated_at_column();

-- Enable realtime for the guestbook tables
